...
#'
#' Searches throughly through each line in a examiner_result to find a
#' call to any of the functions listed in the dots. ...
#'
#' @param ex a examiner_result from a previous application of a examiner function, e.g. `for_examiner()`
...
#' @param message a character string to include in the examiner_result if the search is a failure.
#' @param just_the_fun If TRUE, don't return the whole line, just the call involving the function
#' @return A examiner_result with a pass, fail, or ok result. If a pass, the code component of the
...
#'
#' Like all examiner functions, if the input `ex` is marked as a fail the output will be a short
...
line_calling <- function(ex, ..., n=1L, message = "Didn't find matching function.", just_the_fun = FALSE) {
  ...
  not_yet <- new_examiner_result(action = "fail")
    ...
    res <-
      if (is.null(top_level)) { # the expression isn't a call
        ...
        if (c(top_level) %in% c(qfuns)) {
          ...
          tmp <- new_examiner_result(action = "ok", message = message, code = ex$code[m])
  ...
  new_examiner_result(action = "fail", message = message, code = ex$code)
}
