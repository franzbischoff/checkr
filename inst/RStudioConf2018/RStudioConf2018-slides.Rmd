...
print_function_contents(
  trig_2_check)
#  from_file = system.file("learnr_examples/internal-examples.R", package = "examiner"))
```

Breaking this down, line by line:

- [1] accepts the user submission from `learnr`. The submission is always called `USER_CODE`. The function `for_examiner()` does some pre-processing of the user submission to turn it into evaluated code and format it for use in later `examiner` functions.
...
- [7] To reach here, the student must have used `sin()`. So we'll focus on the angle. You can write specialized `examiner` functions to handle specialized areas. Degrees vs radians will not be an issue in most tutorials.
...
## rlang, redpen, and examiner

### The foundation

`examiner` is based on the `rlang` and `redpen` packages. Two basic technologies:
...
* Available via `install_github("dtkaplan/examiner")`
...
- experience programming with `examiner` may lead to better high-level functions for simplifying checking, like `check_blanks`. We need to dig the *Pit of Success* 
